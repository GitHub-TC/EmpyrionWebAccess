...
namespace FluffySpoon.AspNet.LetsEncrypt.Persistence
{
	class PersistenceService : IPersistenceService
	{
		...
		public async Task PersistSiteCertificateAsync(X509Certificate2 certificate, string privateKey)
		{

			await PersistAsync(SiteCertificateKey, certificate.Export(X509ContentType.SerializedCert, privateKey), _certificatePersistenceStrategies);
            await PersistAsync(SiteCertificateKey + "Key", Encoding.UTF8.GetBytes(privateKey), _certificatePersistenceStrategies);
		...
		public async Task<X509Certificate2> GetPersistedSiteCertificateAsync()
		{
			...
			var keyBytes = await GetPersistedBytesAsync(SiteCertificateKey + "Key", _certificatePersistenceStrategies);
            if (keyBytes == null) return null;
            return new X509Certificate2(bytes, Encoding.UTF8.GetString(keyBytes));
        }
